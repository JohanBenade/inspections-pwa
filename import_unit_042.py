"""
Block 6 Import Script - Unit 042
Inspector: Stemi Tumona (insp-001)
Date: 2026-02-03
Defects: 31 (29 NTS + 2 NI)
Expected: 85 skipped, 31 defects, 407 OK = 523 total
"""
import sqlite3
import uuid
...
DEFECTS = [
    ('c897b472', 'Paint peeling off the top of the door', 'NTS'),
    ('78587133', 'Wet paint at the bottom of door frame', 'NTS'),
    ('cbaefabd', 'Glass needs to be cleaned', 'NTS'),
    ('82751f2d', 'Sand in hinges', 'NTS'),
    ('707304a2', 'Glass needs to be cleaned', 'NTS'),
    ('8ab50f6a', 'Sand in hinges', 'NTS'),
    ('1161cc67', 'The ceiling-mounted light has only one bulb', 'NTS'),
    ('3738af64', 'Locks installed ups and down (One lock facing up and the other facing down when opening)', 'NTS'),
    ('218f3d5a', 'Locks installed ups and down (One lock facing up and the other facing down when opening)', 'NTS'),
    ('b35b9552', 'Top of cupboard needs to be cleaned', 'NTS'),
    ('b6b5d166', 'Paint peeling off the door', 'NTS'),
    ('a6939da2', 'D3 magnetic door is not strong enough', 'NTS'),
    ('0514ada9', 'Glass needs to be cleaned', 'NTS'),
    ('8a7da288', 'Sand in hinges', 'NTS'),
    ('107b77d6', 'Ceiling-mounted light has only one bulb', 'NTS'),
    ('04796e27', 'Grey paint marks on door finish', 'NTS'),
    ('eae7e6e1', 'Door makes a sound when being opened', 'NTS'),
    ('fa540b48', 'Striker plate is loose', 'NTS'),
    ('1dda5625', 'Sand in hinges', 'NTS'),
    ('0a294996', 'Glass needs to be cleaned', 'NTS'),
    ('039bdc70', 'No door stopper by B.I.C. door', 'NI'),
    ('31774738', 'Door makes a sound when being opened', 'NTS'),
    ('6348ca23', 'Sand in hinges', 'NTS'),
    ('59a35f5a', 'Glass needs to be cleaned', 'NTS'),
    ('aef89fec', 'No door stopper by B.I.C. door', 'NI'),
    ('e833bf33', 'Door makes a sound when being opened', 'NTS'),
    ('8ed179fe', 'Sand in hinges', 'NTS'),
    ('3f2f2146', 'Glass needs to be cleaned', 'NTS'),
    ('c0183fae', 'Door makes a sound when being opened', 'NTS'),
    ('1fb635e9', 'Glass needs to be cleaned', 'NTS'),
    ('8638d78f', 'Sand in hinge', 'NTS'),
]
...
def main():
    ...
    if existing_items > 0:
        ...
        for t in templates:
            cur.execute("""INSERT INTO inspection_item
    ...
    cur.execute("""SELECT DISTINCT ii.item_template_id
    ...
    for template_id, raw_desc, dtype in clean_defects:
        ...
        cur.execute("""INSERT INTO defect
    ...
    if new_library:
        ...
        for template_id, cat_name, desc in new_library:
            lib_id = gen_id()
            cur.execute("""INSERT INTO defect_library
    ...
    print(f"Defects: {cur.fetchone()[0]} (expected 31)")
    total = 0
