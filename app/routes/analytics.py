...
def batch_analytics(batch_id):
    ...
    Thin wrapper: calls _build_batch_report_data() so the dashboard and the
    PDF share one data source. Adds dashboard-only extras (batch selector).
    """
    tenant_id = session.get('tenant_id', 'MONOGRAPH')
    ...
    return render_template('analytics/batch_detail.html', **data)
