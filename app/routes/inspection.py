...
def start_inspection(unit_id):
    ...
    for t in templates:
        ...
        # Floor-based auto-exclusion (e.g. burglar bars ground_only)
            ...
            if not prev or prev['status'] == 'pending':
                ...
                # sees what failed last cycle (NI priors have no comment -> None).
                status = 'pending'
    ...
    if cycle['cycle_number'] > 1:
        ...
        # Carry-forward children of Rule 3 parents as ok (unreachable, not actionable)
        db.execute("""
            UPDATE inspection_item SET status = 'ok'
            WHERE inspection_id = ?
            AND status = 'pending'
    ...
    for cat in categories:
        ...
        items_raw = query_db("""
            SELECT it.id as template_id, it.item_description, it.parent_item_id, it.item_order,
                   ii.id, ii.status, ii.comment, ii.marked_at, ii.has_prior_defects,
        ...
        for item in items_raw:
            ...
            checklist.append({
                ...
                'is_carried_ok': item['status'] == 'ok' and item['marked_at'] is None and (item['has_prior_defects'] or 0) == 0,
            })
...
@inspection_bp.route('/<inspection_id>/desnag/latent_address', methods=['POST'])
@require_auth
...
def desnag_submit(inspection_id):
    ...
    # Verify all b/fwd defects AND open latents addressed at this cycle
