...
The fix is structured as a final override pass after all the underlying
maps (defect, latent, items) are populated -- the existing per-stage
assignments still run, then the override runs for C2+ only. C1 units are
completely untouched. unit_checkpoints for C2+ is set equal to checkpoints
to avoid double-counting latents, and total_checkpoints is recomputed.
